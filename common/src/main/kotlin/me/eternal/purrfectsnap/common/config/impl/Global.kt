...
class Global : ConfigContainer() {
    ...
    inner class BetterLocationConfig : ConfigContainer(hasGlobalState = true) {
         ...
         val routingMode = unique("routing_mode", "static", "radius", "route") { addFlags(ConfigFlag.NO_DISABLE_KEY) }
         val routingSpeed = string("routing_speed", "50") { inputCheck = { it.toDoubleOrNull()?.let { s -> s in 1.0..300.0 } == true } }
