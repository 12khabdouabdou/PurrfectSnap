...
object RandomizedDeviceProfileStore {
    private const val prefsName = "purrfect_spoof"
    ...
    /**
     * Force-generates a NEW profile guaranteed to differ from `previous` where
     * the template pool allows (device-model and region exclusions inside
     * [generateProfile]), persists it exactly like [getOrCreate], and returns it.
     *
     * This is the identity-rotation entrypoint backing
     * `DeviceSpooferHook.rotateProfile()` (RotationFSM Tier-1 responder). It
     * deliberately bypasses the getOrCreate cached-profile fast-path so the FSM
     * can never "rotate" to the identity it already holds.
     */
    fun rotate(
        context: Context,
        logger: AbstractLogger,
        generationToken: String?,
        previousProfile: RandomizedDeviceProfile?
    ): RandomizedDeviceProfile {
        val prefs = context.getSharedPreferences(prefsName, Context.MODE_PRIVATE)
        val requestedToken = generationToken.orEmpty()
        // Bounded retry: the `.ifEmpty` fallbacks inside generateProfile can, in a
        // collapsed template pool, re-pick the previous model+region and produce a
        // byte-identical fingerprint. A same-fingerprint "rotation" is a stable
        // identity token that never changed — worse than skipping. Retry up to a
        // small bound; the caller additionally rejects a still-identical profile.
        var profile = generateProfile(previousProfile, null)
        var attempt = 0
        while (previousProfile != null &&
            profile.buildFingerprint == previousProfile.buildFingerprint &&
            attempt < 3
        ) {
            profile = generateProfile(previousProfile, null)
            attempt++
        }
        prefs.edit()
            .putString(profileKey, profile.toJson().toString())
            ...
            .apply()
        logger.info(
            "Rotated randomized device profile ${profile.profileId}: " +
                "${profile.deviceInfo.manufacturer} ${profile.deviceInfo.model}, " +
                "androidId=${profile.androidId}, ip=${profile.ipAddress}, locale=${profile.localeTag}, " +
                "carrier=${profile.simOperatorName}"
        )
        return profile
    }
