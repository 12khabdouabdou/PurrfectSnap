...
import me.eternal.purrfectsnap.bridge.ActiveSortHookStatus
...
class BridgeClient(
    ...
    fun setActiveSortHookStatus(status: ActiveSortHookStatus) = safeServiceCall {
        service.setActiveSortHookStatus(status.name)
    }

    fun getActiveSortStatus(): ActiveSortHookStatus = safeServiceCall {
        runCatching { ActiveSortHookStatus.valueOf(service.activeSortStatus) }.getOrDefault(ActiveSortHookStatus.MAPPER_FAILED)
    }
}
