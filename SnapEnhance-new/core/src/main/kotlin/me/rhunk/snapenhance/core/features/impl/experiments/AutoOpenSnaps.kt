...
class AutoOpenSnaps: MessagingRuleFeature("Auto Open Snaps", MessagingRuleType.AUTO_OPEN_SNAPS) {
    companion object {
        const val ACTION_PAUSE_RESUME = "me.rhunk.snapenhance.AUTO_OPEN_SNAPS_PAUSE_RESUME"
        const val ACTION_CLEAR_QUEUE = "me.rhunk.snapenhance.AUTO_OPEN_SNAPS_CLEAR_QUEUE"
    }
    ...
    private fun verifyQueueSync(): Boolean {
        return synchronized(queuedSnaps) {
            ...
            queuedSnaps.removeAll { item ->
                val isStuck = (currentTime - item.timestamp) > timeoutMs
                if (isStuck) {
                    context.log.warn("Removing stuck item: ${item.messageId}")
                }
                isStuck
            }
            
            if (queuedSnaps.size != originalSize) {
                context.log.warn("Cleaned up ${originalSize - queuedSnaps.size} stuck items")
            }
